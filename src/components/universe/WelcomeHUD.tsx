...
export const WelcomeHUD = () => {
  ...
  return (
    <motion.div 
      ...
      className={`max-w-2xl w-full ${isMobile ? 'p-6' : 'p-12'} bg-black/60 backdrop-blur-3xl border border-white/10 relative overflow-hidden mx-4`}
    >
      ...
      <div className={`relative z-10 ${isMobile ? 'space-y-6' : 'space-y-10'}`}>
        ...
        <div className="text-center space-y-4">
          <motion.div
            ...
            className="inline-flex items-center gap-2 px-3 py-1 bg-cyan-500/10 border border-cyan-500/20 text-cyan-400 text-[10px] sm:text-[12px] font-mono tracking-[0.3em] uppercase mb-4"
          >
            <Info size={12} />
            {t("welcome.guide_ready")}
          </motion.div>
          
          <h1 className={`${isMobile ? 'text-2xl' : 'text-5xl'} font-black text-white tracking-tighter italic whitespace-nowrap`}>
            {t('app.welcome.title')}
          </h1>
          <p className={`${isMobile ? 'text-[10px]' : 'text-sm'} text-zinc-500 font-mono tracking-widest uppercase`}>
        ...
        <div className="grid gap-3 sm:gap-4">
          {features.map((f, i) => (
            <motion.div
              ...
              className={`flex items-start gap-4 ${isMobile ? 'p-2' : 'p-4'} bg-white/5 border border-white/5 hover:border-cyan-500/30 transition-colors group`}
            >
              <div className="text-cyan-500 mt-1 transition-transform group-hover:scale-110 shrink-0">{f.icon}</div>
              <p className={`${isMobile ? 'text-[11px]' : 'text-sm'} text-zinc-400 leading-relaxed`}>{f.text}</p>
            </motion.div>
          ))}
        </div>

        {/* Interaction Guide - hidden on extreme mobile */}
        {!isMobile && (
        ...
        <div className="text-center pt-2 sm:pt-4">
          <div className={`${isMobile ? 'text-[10px]' : 'text-[13px]'} font-mono text-cyan-500 animate-pulse tracking-[0.4em] uppercase`}>
