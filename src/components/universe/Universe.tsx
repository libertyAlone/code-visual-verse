...
const EnergyBeam = ({ start, end, color }: { start: [number, number, number], end: [number, number, number], color: string }) => {
    const curve = useMemo(() => {
...
const DependencyNebula = ({ galaxyNodes }: { galaxyNodes: any[] }) => {
  ...
  return (
    <group>
      {connections.map((c, i) => (
        <EnergyBeam key={`${c.id}-${i}`} start={c.start} end={c.end} color={c.color} />
      ))}
    </group>
  );
};
